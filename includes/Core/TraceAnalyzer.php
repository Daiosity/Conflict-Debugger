...
final class TraceAnalyzer {
	...
	private function empty_trace( array $record, string $key ): array {
		return array(
			'signature'          => md5( $key ),
			'trace_key'          => $key,
			'request_context'    => sanitize_text_field( (string) ( $record['request_context'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ) ),
	...
	private function build_comparison_explanation( array $primary, array $secondary ): string {
		return sprintf(
			...
			__( 'This comparison highlights what changed between the most abnormal captured trace (%1$s) and the closest calmer trace (%2$s).', 'daiosity-conflict-debugger' ),
			(string) ( $primary['label'] ?? __( 'affected trace', 'daiosity-conflict-debugger' ) ),
			(string) ( $secondary['label'] ?? __( 'comparison trace', 'daiosity-conflict-debugger' ) )
		);
	}
	...
	private function build_trace_label( array $trace ): string {
		$context  = sanitize_text_field( (string) ( $trace['request_context'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ) );
		...
		if ( '' !== $resource ) {
			return sprintf(
				...
				__( '%1$s trace on %2$s', 'daiosity-conflict-debugger' ),
				$context,
				$resource
			);
		}
		...
		return sprintf(
			...
			__( '%s trace', 'daiosity-conflict-debugger' ),
			$context
		);
	}
	...
	private function build_trace_summary( array $trace ): string {
		return sprintf(
			...
			__( '%1$d requests, %2$d runtime events, %3$d observed failures, %4$d mutation signals.', 'daiosity-conflict-debugger' ),
