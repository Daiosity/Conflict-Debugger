...
final class ScanStateRepository {
	...
	public function mark_queued( string $token, string $worker_key = '' ): array {
		$state = array(
			'status'        => 'queued',
			'progress'      => 5,
			'message'       => __( 'Scan queued. Waiting for the background worker to begin.', 'daiosity-conflict-debugger' ),
			'token'         => $token,
	...
	public function mark_complete( string $token, int $finding_count ): void {
		...
		$state['token']         = $token;
		$state['progress']      = 100;
		$state['message']       = __( 'Scan complete. Results are ready to review.', 'daiosity-conflict-debugger' );
		$state['updated_at']    = current_time( 'mysql' );
	...
	public function mark_failed( string $token, string $error ): void {
		...
		$state['status']       = 'failed';
		$state['token']        = $token;
		$state['message']      = __( 'The scan did not finish successfully.', 'daiosity-conflict-debugger' );
		$state['updated_at']   = current_time( 'mysql' );
		$state['completed_at'] = current_time( 'mysql' );
	...
	private function default_state(): array {
		return array(
			'status'        => 'idle',
			'progress'      => 0,
			'message'       => __( 'No scan is running.', 'daiosity-conflict-debugger' ),
			'token'         => '',
			'started_at'    => '',
			'updated_at'    => '',
			'completed_at'  => '',
