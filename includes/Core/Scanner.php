...
final class Scanner {
	...
	public function run_scan_with_progress( ?callable $progress_callback = null ): array {
		$this->notify_progress( $progress_callback, __( 'Preparing scan context...', 'daiosity-conflict-debugger' ), 10 );
		$plugins       = $this->get_active_plugins();
		$this->notify_progress( $progress_callback, __( 'Capturing environment details...', 'daiosity-conflict-debugger' ), 30 );
		$environment   = $this->environment->snapshot();
		$this->notify_progress( $progress_callback, __( 'Collecting runtime and log signals...', 'daiosity-conflict-debugger' ), 50 );
		$error_signals = $this->error_collector->collect();
		$this->notify_progress( $progress_callback, __( 'Analyzing plugin interactions...', 'daiosity-conflict-debugger' ), 75 );
		$findings      = $this->detector->detect( $plugins, $error_signals, $environment );
		$this->notify_progress( $progress_callback, __( 'Saving scan results...', 'daiosity-conflict-debugger' ), 90 );
