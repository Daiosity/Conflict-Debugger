...
final class ErrorCollector {
	...
	public function collect(): array {
		...
		if ( ! empty( $signals['log_access']['readable'] ) && '' !== $debug_log ) {
			...
			$signals['notes'][]          = __( 'Direct log access is unavailable. Analysis is based on runtime and plugin interaction signals.', 'daiosity-conflict-debugger' );
		}
		...
		if ( ! empty( $recovery_mode ) ) {
			$signals['notes'][] = __( 'Recovery mode data exists, which may indicate a recent fatal error event.', 'daiosity-conflict-debugger' );
		}
		...
		if ( $signals['trace_summary_count'] > 0 ) {
			$signals['notes'][] = __( 'Trace-level mutation warnings were captured separately from PHP, log, and request error signals.', 'daiosity-conflict-debugger' );
		}
	...
	private function build_log_access_report(): array {
		...
		if ( '' !== $open_basedir ) {
			$recommendations[] = __( 'This server reports open_basedir restrictions. If the log path falls outside that scope, PHP may not be able to read it.', 'daiosity-conflict-debugger' );
		}
	...
	private function deduplicate_entries( array $entries ): array {
		...
		foreach ( $deduplicated as &$entry ) {
			...
			$entry['message'] = sprintf(
				...
				__( 'Repeated %1$d times: %2$s', 'daiosity-conflict-debugger' ),
				$repeat_count,
