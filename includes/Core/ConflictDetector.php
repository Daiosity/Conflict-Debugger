...
final class ConflictDetector {
		...
		for ( $i = 0, $plugin_count = count( $plugins ); $i < $plugin_count; $i++ ) {
			for ( $j = $i + 1; $j < $plugin_count; $j++ ) {
				...
				if ( ! empty( $error_match ) ) {
					...
					$observed_context      = sanitize_text_field( (string) ( $observed_entry['request_context'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ) );
					...
					$global_evidence[] = $this->create_evidence_item(
						...
						array(
							...
							'request_context'   => '' !== $observed_context ? $observed_context : __( 'runtime', 'daiosity-conflict-debugger' ),
					...
					if ( ! empty( $observed_entry['contaminated'] ) ) {
						$global_evidence[] = $this->create_evidence_item(
							...
							__( 'The runtime clue includes third-party screen or resource context, so it was treated as contaminated support instead of direct pair-specific proof.', 'daiosity-conflict-debugger' ),
							'weak',
							array(
								'tier'              => 'noise',
								'request_context'   => '' !== $observed_context ? $observed_context : __( 'runtime', 'daiosity-conflict-debugger' ),
				...
				if ( ! empty( $plugin_a['recently_changed'] ) || ! empty( $plugin_b['recently_changed'] ) ) {
					$global_evidence[] = $this->create_evidence_item(
						'generic',
						'recent_change',
						__( 'One or both plugins changed recently. That can help narrow the timeline, but it is not proof of a conflict by itself.', 'daiosity-conflict-debugger' ),
						'weak',
						array(
							'tier'            => 'weak',
							'request_context' => __( 'runtime', 'daiosity-conflict-debugger' ),
						)
					);
				}
				...
				$findings[] = array(
					...
					'affected_area'                     => (string) ( $surface_meta['affected_area'] ?? __( 'generic site behavior', 'daiosity-conflict-debugger' ) ),
	...
	private function get_surface_definitions(): array {
		return array(
			'frontend_rendering' => array(
				'label'         => __( 'Frontend Rendering', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'frontend rendering', 'daiosity-conflict-debugger' ),
			...
			'asset_loading' => array(
				'label'         => __( 'Asset Loading', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'asset loading', 'daiosity-conflict-debugger' ),
			...
			'admin_screen' => array(
				'label'         => __( 'Admin Screen', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'admin settings', 'daiosity-conflict-debugger' ),
				...
				'hook_prefixes' => array(
					...
					array( 'prefix' => 'load-', 'signal_key' => 'admin_screen_overlap', 'template' => __( 'Both plugins attach to the same admin page load hook: %s.', 'daiosity-conflict-debugger' ) ),
					/* translators: %s: admin postback hook name. */
					array( 'prefix' => 'admin_post_', 'signal_key' => 'admin_screen_overlap', 'template' => __( 'Both plugins attach to the same admin save or postback action: %s.', 'daiosity-conflict-debugger' ) ),
				),
				'categories'    => array( 'admin-tools' ),
				'context_terms' => array( 'settings', 'admin', 'options', 'screen', 'save settings', 'tools page', 'menu slug' ),
			),
			'editor' => array(
				'label'         => __( 'Editor', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'editor', 'daiosity-conflict-debugger' ),
			...
			'authentication_account' => array(
				'label'         => __( 'Authentication / Account', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'login', 'daiosity-conflict-debugger' ),
			...
			'rest_api_ajax' => array(
				'label'         => __( 'REST API / AJAX', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'API/AJAX', 'daiosity-conflict-debugger' ),
				'signal_key'    => 'surface_hook_overlap',
				'hooks'         => array( 'rest_api_init' ),
				'hook_prefixes' => array(
					/* translators: %s: authenticated AJAX action name. */
					array( 'prefix' => 'wp_ajax_', 'signal_key' => 'ajax_action_overlap', 'template' => __( 'Both plugins register the same authenticated AJAX action: %s.', 'daiosity-conflict-debugger' ) ),
					/* translators: %s: public AJAX action name. */
					array( 'prefix' => 'wp_ajax_nopriv_', 'signal_key' => 'ajax_action_overlap', 'template' => __( 'Both plugins register the same public AJAX action: %s.', 'daiosity-conflict-debugger' ) ),
				),
				'categories'    => array( 'api' ),
				'context_terms' => array( 'rest', 'ajax', 'nonce', 'endpoint', 'api', 'request' ),
			),
			'forms_submission' => array(
				'label'         => __( 'Forms / Submission Workflows', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'forms', 'daiosity-conflict-debugger' ),
				'signal_key'    => 'surface_hook_overlap',
				'hooks'         => array( 'template_redirect', 'init' ),
				'hook_prefixes' => array(
					/* translators: %s: admin postback hook name. */
					array( 'prefix' => 'admin_post_', 'signal_key' => 'exact_hook_collision', 'template' => __( 'Both plugins appear to process the same submit/postback action: %s.', 'daiosity-conflict-debugger' ) ),
				),
				'categories'    => array( 'forms' ),
				'context_terms' => array( 'form', 'submit', 'validation', 'captcha', 'spam', 'processing' ),
			),
			'caching_optimization' => array(
				'label'         => __( 'Caching / Optimization', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'caching', 'daiosity-conflict-debugger' ),
			...
			'seo_metadata' => array(
				'label'         => __( 'SEO / Metadata', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'SEO', 'daiosity-conflict-debugger' ),
			...
			'rewrite_routing' => array(
				'label'         => __( 'Rewrite / Routing', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'routing', 'daiosity-conflict-debugger' ),
			...
			'content_model' => array(
				'label'         => __( 'Content Model', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'content model', 'daiosity-conflict-debugger' ),
			...
			'email_notifications' => array(
				'label'         => __( 'Email / Notifications', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'notifications', 'daiosity-conflict-debugger' ),
			...
			'security_access' => array(
				'label'         => __( 'Security / Access Control', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'security', 'daiosity-conflict-debugger' ),
			...
			'background_processing' => array(
				'label'         => __( 'Cron / Background Jobs', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'background processing', 'daiosity-conflict-debugger' ),
			...
			'commerce_checkout' => array(
				'label'         => __( 'Commerce / Checkout', 'daiosity-conflict-debugger' ),
				'affected_area' => __( 'forms', 'daiosity-conflict-debugger' ),
				'signal_key'    => 'surface_hook_overlap',
	...
	private function analyze_hook_surface_overlaps(): array {
		...
		$default_surface_template = __( 'Both plugins attach to the %s hook.', 'daiosity-conflict-debugger' );
					...
					for ( $j = $i + 1; $j < $count; $j++ ) {
						...
						if ( $extreme_priority ) {
							$this->add_analysis_item(
								...
								(string) $surface_key,
								'extreme_priority',
								__( 'One or more callbacks on this surface run at extreme priorities, which can change load order in hard-to-debug ways.', 'daiosity-conflict-debugger' ),
								'weak',
								array(
									'tier'              => 'noise',
									'request_context'   => $this->request_context_for_surface( (string) $surface_key ),
				...
				for ( $j = $i + 1; $j < $count; $j++ ) {
					...
					$this->add_analysis_item(
						...
						sprintf(
							...
							__( 'Both plugins register assets from the same library family: %s.', 'daiosity-conflict-debugger' ),
							$family
						),
						'contextual',
						array(
				...
				for ( $j = $i + 1; $j < $count; $j++ ) {
					...
					$this->add_analysis_item(
						...
						sprintf(
							...
							__( 'Both plugins appear to modify the same optimization layer: %s.', 'daiosity-conflict-debugger' ),
							$flag
						),
						'contextual',
						array(
	...
	private function analyze_cron_surface_overlaps( array $plugins ): array {
		...
		foreach ( (array) $cron_array as $events ) {
			foreach ( (array) $events as $hook_name => $instances ) {
				...
				$family_name = __( 'scheduled processing', 'daiosity-conflict-debugger' );
					...
					for ( $j = $i + 1; $j < $count; $j++ ) {
						...
						$this->add_analysis_item(
							...
							sprintf(
								...
								__( 'Both plugins appear to schedule similar background work around %1$s (%2$s).', 'daiosity-conflict-debugger' ),
								$hook_name,
								$family_name
							),
							'contextual',
		...
		foreach ( $post_type_regs as $post_type => $registrations ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins register the same custom post type key: %s.', 'daiosity-conflict-debugger' ),
					$post_type
				)
			);
		...
		foreach ( $taxonomy_regs as $taxonomy => $registrations ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins register the same taxonomy key: %s.', 'daiosity-conflict-debugger' ),
					$taxonomy
				)
			);
		...
		foreach ( $rewrite_slug_registry as $rewrite_slug => $registrations ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins register the same rewrite slug: %s.', 'daiosity-conflict-debugger' ),
					$rewrite_slug
				)
			);
		}
		...
		foreach ( $rest_base_registry as $rest_base => $registrations ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins expose the same REST base or API content base: %s.', 'daiosity-conflict-debugger' ),
					$rest_base
				)
			);
		}
		...
		foreach ( $query_var_registry as $query_var => $registrations ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins rely on the same query var: %s.', 'daiosity-conflict-debugger' ),
					$query_var
				)
			);
		}
		...
		foreach ( $menu_slug_registry as $menu_slug => $menu_items ) {
			$this->add_exact_registry_pairs(
				...
				sprintf(
					...
					__( 'Both plugins register the same admin menu or page slug: %s.', 'daiosity-conflict-debugger' ),
					$menu_slug
				)
			);
		}
			...
			for ( $i = 0, $count = count( $owner_slugs ); $i < $count; $i++ ) {
				for ( $j = $i + 1; $j < $count; $j++ ) {
					$this->add_analysis_item(
						...
						$surface_key,
						$signal_key,
						'' !== $message ? $message : __( 'Observed runtime callback or asset mutation involving both plugins.', 'daiosity-conflict-debugger' ),
						$strength,
						array(
							'tier'            => $tier,
		...
		foreach ( $this->surfaces as $surface_key => $surface ) {
			...
			$this->add_surface_evidence(
				...
				sprintf(
					...
					__( '%1$s and %2$s both operate in the %3$s surface.', 'daiosity-conflict-debugger' ),
		...
		foreach ( $this->surfaces as $surface_key => $surface ) {
			...
			$this->add_surface_evidence(
				...
				sprintf(
					...
					__( 'Runtime clues align with the %1$s surface (%2$s).', 'daiosity-conflict-debugger' ),
					(string) $surface['label'],
	...
	private function detect_extension_relationship( array $plugin_a, array $plugin_b ): array {
		...
		if ( ( $has_dependency_keywords && $mentions_other ) || ( $has_dependency_keywords && $shared_prefix ) ) {
			return array(
				'is_extension' => true,
				'message'      => __( 'These plugins look like a parent/addon pair. Shared hooks and categories may be expected unless stronger runtime signals also correlate.', 'daiosity-conflict-debugger' ),
			);
		}
	...
	private function format_observed_breakage_message( array $entry ): string {
		$request_context = sanitize_text_field( (string) ( $entry['request_context'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ) );
		...
		if ( 'generic_runtime_noise' === $signal_type ) {
			return sprintf(
				...
				__( 'Supporting runtime noise was observed in the %1$s context around %2$s, but the failing path was not proven to be pair-specific.', 'daiosity-conflict-debugger' ),
				$request_context,
				'' !== $execution_surface ? $execution_surface : __( 'the active execution path', 'daiosity-conflict-debugger' )
			);
		}
		...
		if ( $status_code >= 400 ) {
			return sprintf(
				...
				__( 'Observed %2$d response in the %1$s context during runtime telemetry.', 'daiosity-conflict-debugger' ),
				$request_context,
				$status_code
			);
		}
		...
		if ( '' !== $resource ) {
			return sprintf(
				...
				__( 'Observed breakage in the %1$s context involving %2$s on %3$s.', 'daiosity-conflict-debugger' ),
				$request_context,
				$resource,
				'' !== $execution_surface ? $execution_surface : __( 'the active execution path', 'daiosity-conflict-debugger' )
			);
		...
		return sprintf(
			...
			__( 'Observed breakage in the %1$s context: %2$s', 'daiosity-conflict-debugger' ),
			$request_context,
	...
	private function build_grouped_anomaly_findings( array $pattern_groups, array $plugin_index ): array {
		...
		foreach ( $pattern_groups as $pattern_group ) {
			...
			$request_context   = sanitize_text_field( (string) ( $sample_entry['request_context'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ) );
			...
			$evidence_items = array(
				$this->create_evidence_item(
					...
					sprintf(
						...
						__( '%1$s shows the same callback-chain churn pattern against %2$d different plugins.', 'daiosity-conflict-debugger' ),
				...
				$this->create_evidence_item(
					...
					__( 'The repeated fingerprint is based on callback presence differences across snapshots, which can happen with conditional or observer-driven lifecycle behavior.', 'daiosity-conflict-debugger' ),
					'context',
					array(
						'tier'              => 'contextual',
						'request_context'   => $request_context,
			...
			$findings[] = array(
				...
				'secondary_plugin_name'            => sprintf(
					/* translators: %d count. */
					__( 'Multiple plugins (%d)', 'daiosity-conflict-debugger' ),
					count( $counterpart_slugs )
				),
				'issue_category'                   => $surface_key,
				'surface_key'                      => $surface_key,
				'surface_label'                    => (string) ( $this->surfaces[ $surface_key ]['label'] ?? $surface_key ),
				'affected_area'                    => (string) ( $this->surfaces[ $surface_key ]['affected_area'] ?? __( 'runtime', 'daiosity-conflict-debugger' ) ),
				'title'                            => ! empty( $pattern_group['is_observer'] )
					? sprintf(
						/* translators: %s plugin name. */
						__( 'Recurring runtime anomaly involving %s', 'daiosity-conflict-debugger' ),
						(string) ( $anchor_plugin['name'] ?? $anchor_slug )
					)
					: __( 'Repeated callback-chain interference pattern detected', 'daiosity-conflict-debugger' ),
				'severity'                         => $severity,
				...
				'explanation'                      => ! empty( $pattern_group['is_observer'] )
					? sprintf(
						...
						__( '%1$s appears in the same callback-chain churn pattern across multiple unrelated plugins in the %2$s context on %3$s. That is more consistent with observer-plugin callback churn or snapshot timing noise than a uniquely confirmed pairwise conflict.', 'daiosity-conflict-debugger' ),
						(string) ( $anchor_plugin['name'] ?? $anchor_slug ),
						$request_context,
						'' !== $execution_surface ? $execution_surface : __( 'the active execution path', 'daiosity-conflict-debugger' )
					)
					: sprintf(
						/* translators: 1: request context, 2: execution surface. */
						__( 'The same non-specific callback-chain churn pattern repeats across multiple plugin pairs in the %1$s context on %2$s. This should be treated as a global anomaly until a single shared resource or remover is identified.', 'daiosity-conflict-debugger' ),
						$request_context,
						'' !== $execution_surface ? $execution_surface : __( 'the active execution path', 'daiosity-conflict-debugger' )
					),
				'why_scored_this_way'              => ! empty( $pattern_group['is_observer'] )
					? __( 'Scored as an observer artifact because the same callback-chain churn fingerprint repeats against multiple unrelated plugins instead of isolating one pair-specific resource collision.', 'daiosity-conflict-debugger' )
					: __( 'Scored as a global anomaly because the repeated runtime pattern is broad and non-specific across several plugin pairs.', 'daiosity-conflict-debugger' ),
				'why_this_is_not_or_is_actionable' => ! empty( $pattern_group['is_observer'] )
					? __( 'This is not a confirmed pairwise conflict. Reproduce the affected request with the observer/debug plugin disabled or compare traces from the same request path before escalating to a plugin-pair diagnosis.', 'daiosity-conflict-debugger' )
					: __( 'This is actionable as a runtime anomaly review, not as a confirmed plugin-pair conflict. Look for a single shared resource or exact remover before treating any pair as confirmed.', 'daiosity-conflict-debugger' ),
				'recommended_next_step'            => ! empty( $pattern_group['is_observer'] )
					? __( 'Reproduce the affected request in staging with the observer/debug plugin disabled, then compare the callback chain and runtime failures on the same request path.', 'daiosity-conflict-debugger' )
					: __( 'Capture the same request path again and isolate a single shared resource, callback identifier, or remover before acting on any pairwise diagnosis.', 'daiosity-conflict-debugger' ),
		...
		foreach ( $this->known_risk_rules as $anchor => $keywords ) {
			...
			foreach ( $keywords as $keyword ) {
				if ( false !== strpos( $combined, $keyword ) ) {
					return __( 'This combination matches an internal risk pattern that often deserves manual compatibility testing.', 'daiosity-conflict-debugger' );
				}
			}
		}
	...
	private function build_explanation( array $plugin_a, array $plugin_b, string $finding_type, string $surface_key, string $shared_resource, string $request_context, string $execution_surface = '' ): string {
		...
		$area    = (string) ( $surface['affected_area'] ?? __( 'generic site behavior', 'daiosity-conflict-debugger' ) );
		$context = $request_context ? $request_context : $area;
		$surface_note = '' !== $execution_surface ? ' ' . sprintf(
			/* translators: %s execution surface. */
			__( 'Execution surface: %s.', 'daiosity-conflict-debugger' ),
			$execution_surface
		) : '';
		...
		if ( 'confirmed_conflict' === $finding_type ) {
			return sprintf(
				...
				__( 'Confirmed runtime breakage was observed for %1$s and %2$s in the %3$s context. Direct evidence points to %4$s on the live execution path.', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				(string) $plugin_b['name'],
				$context,
				$shared_resource ? $shared_resource : __( 'this runtime path', 'daiosity-conflict-debugger' )
			) . $surface_note;
		}
		...
		if ( 'probable_conflict' === $finding_type ) {
			return sprintf(
				...
				__( 'Direct evidence suggests %1$s and %2$s both affect %3$s in the %4$s context. This goes beyond broad overlap, but it still needs validation on the affected request path.', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				(string) $plugin_b['name'],
				$shared_resource ? $shared_resource : __( 'the same resource', 'daiosity-conflict-debugger' ),
				$context
			) . $surface_note;
		}
		...
		if ( 'potential_interference' === $finding_type ) {
			return sprintf(
				...
				__( '%1$s and %2$s show a potential interference pattern in the %3$s context. Supporting indicators line up on the same execution path, but the proof is still incomplete.', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				(string) $plugin_b['name'],
				$context
			) . $surface_note;
		...
		if ( 'observer_artifact' === $finding_type ) {
			return sprintf(
				...
				__( 'A recurring runtime anomaly involving %1$s was observed in the %2$s context. The repeated fingerprint looks more like observer-plugin callback churn or snapshot timing noise than a unique pair-specific conflict.', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				$context
			) . $surface_note;
		}
		...
		if ( 'global_anomaly' === $finding_type ) {
			return sprintf(
				...
				__( 'The same non-specific runtime pattern repeats across multiple plugin pairs in the %s context. Treat it as a global anomaly until a single shared resource or remover is identified.', 'daiosity-conflict-debugger' ),
				$context
			) . $surface_note;
		}
		...
		if ( 'shared_surface' === $finding_type ) {
			return sprintf(
				/* translators: 1: plugin A, 2: plugin B, 3: request context. */
				__( '%1$s and %2$s touch the same execution surface in the %3$s context. That matters more than broad overlap, but no exact shared resource or direct interference is proven yet.', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				(string) $plugin_b['name'],
				$context
			) . $surface_note;
		...
		return sprintf(
			/* translators: 1: plugin A, 2: plugin B, 3: request context. */
			__( '%1$s and %2$s were observed on a shared runtime surface in the %3$s context. That is common in WordPress and is not direct conflict evidence on its own.', 'daiosity-conflict-debugger' ),
			(string) $plugin_a['name'],
			(string) $plugin_b['name'],
			$context
		) . $surface_note;
	...
	private function build_title( array $plugin_a, array $plugin_b, string $finding_type, string $surface_key, string $shared_resource, string $request_context ): string {
		...
		if ( 'global_anomaly' === $finding_type ) {
			return __( 'Repeated callback-chain interference pattern detected', 'daiosity-conflict-debugger' );
		}

		if ( 'confirmed_conflict' === $finding_type ) {
			return sprintf(
				/* translators: 1: plugin A, 2: plugin B. */
				__( 'Confirmed runtime breakage involving %1$s and %2$s', 'daiosity-conflict-debugger' ),
				(string) $plugin_a['name'],
				(string) $plugin_b['name']
			);
		}
		...
		if ( 'probable_conflict' === $finding_type ) {
			return sprintf(
				...
				__( 'Probable conflict on %1$s in %2$s', 'daiosity-conflict-debugger' ),
		...
		if ( 'potential_interference' === $finding_type ) {
			return sprintf(
				/* translators: %s request context. */
				__( 'Potential interference in %s', 'daiosity-conflict-debugger' ),
				$request_context
			);
		}
		...
		if ( 'shared_surface' === $finding_type ) {
			return sprintf(
				...
				__( 'Shared execution surface in %s', 'daiosity-conflict-debugger' ),
				$surface_label
			);
		}
		...
		return sprintf(
			/* translators: %s surface label. */
			__( 'Normal overlap on %s', 'daiosity-conflict-debugger' ),
			$surface_label
		);
	}
	...
	private function build_actionability_note( string $finding_type, string $shared_resource, string $request_context, string $execution_surface = '' ): string {
		if ( 'confirmed_conflict' === $finding_type ) {
			return __( 'This is actionable because the scan observed runtime breakage rather than just shared plugin presence.', 'daiosity-conflict-debugger' );
		}

		if ( 'probable_conflict' === $finding_type ) {
			return sprintf(
				/* translators: 1: shared resource, 2: request context. */
				__( 'This is actionable because direct shared-resource evidence points to %1$s in the same %2$s context.', 'daiosity-conflict-debugger' ),
				$shared_resource ? $shared_resource : __( 'the same resource', 'daiosity-conflict-debugger' ),
				$request_context
			);
		}
		...
		if ( 'potential_interference' === $finding_type ) {
			return __( 'This is worth validating on the affected request path, but it is not yet proof that one plugin is breaking the other.', 'daiosity-conflict-debugger' );
		}

		if ( 'observer_artifact' === $finding_type ) {
			return __( 'This is not actionable as a confirmed plugin-pair conflict. Verify the same request without the observer/debug plugin before treating it as pair-specific interference.', 'daiosity-conflict-debugger' );
		}

		if ( 'global_anomaly' === $finding_type ) {
			return __( 'This is actionable as a global runtime anomaly review, not as proof that any one plugin pair is confirmed.', 'daiosity-conflict-debugger' );
		}

		if ( 'shared_surface' === $finding_type ) {
			return __( 'This is supporting context only. It becomes useful if a later trace shows a concrete shared resource or runtime failure on the same path.', 'daiosity-conflict-debugger' );
		}

		return __( 'This is broad overlap only. Treat it as background context, not as proof of a conflict.', 'daiosity-conflict-debugger' );
	}
	...
	private function build_recommended_next_step( string $surface_key, string $request_context, string $execution_surface, string $shared_resource, array $evidence_items ): string {
		if ( $this->evidence_has_signal( $evidence_items, array( 'direct_callback_mutation' ) ) ) {
			return sprintf(
				...
				__( 'Inspect remove_action and remove_filter activity on %1$s, then replay the affected %3$s request while tracing the callback chain for %2$s.', 'daiosity-conflict-debugger' ),
				'' !== $execution_surface ? $execution_surface : __( 'the affected hook', 'daiosity-conflict-debugger' ),
				'' !== $shared_resource ? $shared_resource : __( 'the removed callback', 'daiosity-conflict-debugger' ),
				'' !== $request_context ? $request_context : __( 'runtime', 'daiosity-conflict-debugger' )
			);
		}
		...
		if ( $this->evidence_has_signal( $evidence_items, array( 'callback_order_sensitivity', 'callback_chain_churn' ) ) ) {
			return sprintf(
				...
				__( 'Trace callback order on %1$s and compare priorities across the affected %2$s request before treating this as a confirmed conflict.', 'daiosity-conflict-debugger' ),
				'' !== $execution_surface ? $execution_surface : __( 'the affected hook', 'daiosity-conflict-debugger' ),
				'' !== $request_context ? $request_context : __( 'runtime', 'daiosity-conflict-debugger' )
			);
		}
		...
		if ( $this->evidence_has_signal( $evidence_items, array( 'asset_state_mutation' ) ) ) {
			return sprintf(
				...
				__( 'Trace the asset lifecycle for %1$s and compare registration, queue, and final state on the affected %2$s request.', 'daiosity-conflict-debugger' ),
				'' !== $shared_resource ? $shared_resource : __( 'the affected handle', 'daiosity-conflict-debugger' ),
				'' !== $request_context ? $request_context : __( 'runtime', 'daiosity-conflict-debugger' )
			);
		}
	...
	private function request_context_for_surface( string $surface_key ): string {
		$contexts = array(
			'frontend_rendering'     => __( 'frontend', 'daiosity-conflict-debugger' ),
			'asset_loading'          => __( 'frontend', 'daiosity-conflict-debugger' ),
			'admin_screen'           => __( 'admin', 'daiosity-conflict-debugger' ),
			'editor'                 => __( 'editor', 'daiosity-conflict-debugger' ),
			'authentication_account' => __( 'login', 'daiosity-conflict-debugger' ),
			'rest_api_ajax'          => __( 'REST/AJAX', 'daiosity-conflict-debugger' ),
			'forms_submission'       => __( 'forms', 'daiosity-conflict-debugger' ),
			'caching_optimization'   => __( 'frontend', 'daiosity-conflict-debugger' ),
			'seo_metadata'           => __( 'frontend', 'daiosity-conflict-debugger' ),
			'rewrite_routing'        => __( 'routing', 'daiosity-conflict-debugger' ),
			'content_model'          => __( 'content model', 'daiosity-conflict-debugger' ),
			'email_notifications'    => __( 'notifications', 'daiosity-conflict-debugger' ),
			'security_access'        => __( 'login/admin', 'daiosity-conflict-debugger' ),
			'background_processing'  => __( 'cron', 'daiosity-conflict-debugger' ),
			'commerce_checkout'      => __( 'checkout/cart', 'daiosity-conflict-debugger' ),
		);

		return $contexts[ $surface_key ] ?? __( 'runtime', 'daiosity-conflict-debugger' );
	}
	...
	private function extract_shared_resource( string $signal_key, string $message ): string {
		...
		if ( 'recent_change' === $signal_key ) {
			return __( 'recent plugin changes', 'daiosity-conflict-debugger' );
		}
	...
	private function classify_optimization_asset( string $source, string $handle ): ?string {
		...
		if ( false !== strpos( $text, 'lazy' ) ) {
			return __( 'lazy loading', 'daiosity-conflict-debugger' );
		}

		if ( false !== strpos( $text, 'defer' ) || false !== strpos( $text, 'delay' ) ) {
			return __( 'script deferral/delay', 'daiosity-conflict-debugger' );
		}

		if ( false !== strpos( $text, 'min' ) && ( false !== strpos( $text, 'css' ) || false !== strpos( $text, 'js' ) ) ) {
			return __( 'minification/deferred assets', 'daiosity-conflict-debugger' );
		}
